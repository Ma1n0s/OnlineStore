...
const { data: advantage } = await useAsyncData(`advantage-${route.params.id}`, () => 
  $fetch(`${backendUrl}/api/advantages/${route.params.id}`)
)

const form = ref({
  title: advantage.value?.title || '',
  icon: advantage.value?.icon || '',
  description: advantage.value?.description || '',
  link: advantage.value?.link || '',
  is_special: advantage.value?.is_special || false,
  tags: advantage.value?.tags || [] as string[],
  sort_order: advantage.value?.sort_order || 0
})
...
const submit = async () => {
  try {
    await $fetch(`${backendUrl}/api/advantages/${route.params.id}`, {
      method: 'PUT',
      body: form.value
    })
...
<template>
  <div class="container mx-auto px-4 py-8">
    <h1 class="text-2xl font-bold mb-8">Редактирование преимущества</h1>
