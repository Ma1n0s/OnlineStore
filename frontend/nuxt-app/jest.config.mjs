...
const config = {
  ...
  // Указываем пресет для TypeScript
  preset: 'ts-jest',
  ...
  // Указываем трансформеры
  transform: {
    '^.+\\.[jt]sx?$': 'babel-jest',
    '^.+\\.(png|jpg|jpeg|gif|webp|svg|ico|ttf|woff|woff2|eot|otf)$': 'jest-transform-stub',
  },

  // Сопоставление путей для алиасов Nuxt
  moduleNameMapper: {
    '^~/(.*)$': '<rootDir>/$1',
    '^@/(.*)$': '<rootDir>/$1',
